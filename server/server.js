...
const ensureSchema = async () => {
    const statements = [
        ...
        // The very first admin is created with a password the operator did not
        // choose (generated by the installer or by ensureFirstAdmin). That
        // credential must not survive the first sign-in: this flag forces the
        // change-password screen before any admin API will answer. Defaults
        // false so every existing account is unaffected.
...
const ensureFirstAdmin = async () => {
    try {
        ...
        // must_change_password = true unconditionally. Even when the operator
        // set ADMIN_PASSWORD (the installer does), that value lives in .env in
        // plain text and was typed at a shell — it is a bootstrap secret, not a
        // credential the human has committed to. First sign-in forces a new one.
        await db.query(
            `INSERT INTO users (username, password_hash, role, email, full_name, must_change_password)
             VALUES ('admin', $1, 'admin', 'admin@localhost', 'System Administrator', true)`,
        ...
        if (generated) {
            ...
            console.log('  Temporary — you are asked to set your own at first sign-in.');
            console.log('  Set ADMIN_PASSWORD before first start to choose the temporary one.');
            console.log('='.repeat(64) + '\n');
        } else {
            console.log('First administrator "admin" created from ADMIN_PASSWORD (temporary; '
